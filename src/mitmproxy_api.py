...
class mitmproxy_api():
    ...
    def getDrHeaderData(self):
        ...
        for item in self.info:
            ...
            listUrlsAndHeaders.append(dictData.copy())
            dictData = {}
        with open ('drheader_data.txt', 'w') as file:
            file.write(json.dumps(listUrlsAndHeaders))
        file.close()
        return subprocess.check_output(["drheader", "scan", "bulk", "drheader_data.txt", "--json"]).decode("utf-8")
